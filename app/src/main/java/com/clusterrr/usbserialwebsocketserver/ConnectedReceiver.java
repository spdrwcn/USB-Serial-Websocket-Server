...
public class ConnectedReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (BuildConfig.DEBUG) {
            Log.d(UsbSerialWebsocketService.TAG, "Connected device detected");
        }
        ...
        {
            switch (UsbSerialWebsocketService.getDeviceStatus(context)) {
                case NO_DEVICE:
                    Log.wtf(UsbSerialWebsocketService.TAG, context.getString(R.string.device_not_found));
                    return;
                case NO_PERMISSION:
                    Log.e(UsbSerialWebsocketService.TAG, context.getString(R.string.missing_usb_device_permission));
